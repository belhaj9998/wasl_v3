...
async function seedPermissions() {
    ...
    console.log(`  ✓ ${constants_1.permissionSeeds.length} permissions seeded`);
}
async function seedSubscriptionPlans() {
    ...
    console.log(`  ✓ ${constants_1.subscriptionPlans.length} plans seeded`);
}
async function seedUsers() {
    console.log("  Seeding users...");
    const createdUsers = [];
    for (const userData of demoUsers) {
        const hashedPassword = await bcryptjs_1.default.hash(userData.password, BCRYPT_ROUNDS);
...
async function seedRolesAndMemberships(stores, users) {
    ...
    const eleganceStore = stores.find((s) => s.domain === "elegance-store");
    const techStore = stores.find((s) => s.domain === "tech-store");
    const ahmed = users.find((u) => u.email === "ahmed@store.com");
    const sara = users.find((u) => u.email === "sara@store.com");
    if (eleganceStore && ahmed) {
        const ownerRole = await prisma_1.default.storeRole.findUnique({
            where: { store_id_slug: { store_id: eleganceStore.id, slug: "owner" } },
        });
        if (ownerRole) {
            await prisma_1.default.storeMembership.upsert({
                where: {
    ...
    if (eleganceStore && sara) {
        const staffRole = await prisma_1.default.storeRole.findUnique({
...
async function seedSubscriptions(stores) {
    console.log("  Seeding subscriptions...");
    const starterPlan = await prisma_1.default.subscriptionPlan.findUnique({
        where: { code: "starter" },
    });
    const growthPlan = await prisma_1.default.subscriptionPlan.findUnique({
        where: { code: "growth" },
    });
    const eleganceStore = stores.find((s) => s.domain === "elegance-store");
    const techStore = stores.find((s) => s.domain === "tech-store");
    if (eleganceStore && growthPlan) {
        await prisma_1.default.storeSubscription.upsert({
    ...
    if (techStore && starterPlan) {
        await prisma_1.default.storeSubscription.upsert({
            ...
            create: {
                ...
                current_period_starts_at: new Date(),
                current_period_ends_at: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000),
            },
        });
    }
    console.log("  ✓ Subscriptions seeded");
}
async function seedCategories(stores) {
    console.log("  Seeding categories...");
    for (const catData of demoCategories) {
        const store = stores.find((s) => s.domain === catData.storeDomain);
        if (!store)
            continue;
...
async function seedProducts(stores) {
    console.log("  Seeding products...");
    for (const prodData of demoProducts) {
        const store = stores.find((s) => s.domain === prodData.storeDomain);
        if (!store)
            continue;
        ...
        if (category) {
            await prisma_1.default.productCategory.create({
                data: {
                    store_id: store.id,
        ...
        if (prodData.track_inventory) {
            ...
            await prisma_1.default.inventory.create({
                data: {
                    store_id: store.id,
...
async function seedCoupons(stores) {
    console.log("  Seeding coupons...");
    for (const couponData of demoCoupons) {
        const store = stores.find((s) => s.domain === couponData.storeDomain);
        if (!store)
            continue;
        await prisma_1.default.coupon.upsert({
...
async function seedCustomers(stores) {
    ...
    for (const custData of demoCustomers) {
        const store = stores.find((s) => s.domain === custData.storeDomain);
        if (!store)
            continue;
        const hashedPassword = await bcryptjs_1.default.hash(custData.password, BCRYPT_ROUNDS);
        const customer = await prisma_1.default.customer.upsert({
            where: { store_id_email: { store_id: store.id, email: custData.email } },
            update: {},
            create: {
                store_id: store.id,
        ...
        await prisma_1.default.customerAddress.create({
            data: {
                store_id: store.id,
...
async function main() {
    ...
    console.log("├─────────────────────────────────────────────────────────────────┤");
    console.log("│ Platform Owner    │ owner@wasl.com     │ Owner123!   │ Full access│");
    console.log("│ Platform Admin    │ admin@wasl.com     │ Admin123!   │ Platform   │");
    console.log("│ Store Owner       │ ahmed@store.com    │ Ahmed123!   │ Both stores│");
    console.log("│ Store Staff       │ sara@store.com     │ Sara1234!   │ Store 1    │");
    console.log("└─────────────────────────────────────────────────────────────────┘");
    console.log("\n🏪 Stores:");
